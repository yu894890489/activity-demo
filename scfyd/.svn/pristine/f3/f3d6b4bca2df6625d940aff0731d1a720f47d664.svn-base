...
import com.ut.scf.reqbean.crm.CashFlowAddReqBean;
import com.ut.scf.reqbean.crm.CashFlowListReqBean;
import com.ut.scf.reqbean.crm.CashFlowUpdateReqBean;
...
public class CashFlowController extends BaseJsonController {
	@Resource
	...
	public @ResponseBody BaseRespBean CashFlowList(HttpSession httpSession,
			@RequestBody CashFlowListReqBean reqBean,
...
//			}
		...
		return cashflowService.getCashFlowList(paramMap, page);

	}

	@RequestMapping(value = "/add", method = RequestMethod.POST, consumes = { "application/json" }, produces = { "application/json;charset=UTF-8" })
	public @ResponseBody BaseRespBean CashFlowAdd(HttpSession httpSession,
			@Valid @RequestBody CashFlowAddReqBean reqBean,
			BindingResult bindingResult) throws IOException {
	...
	public @ResponseBody BaseRespBean CashFlowDelete(HttpSession httpSession,
		...
		respBean = cashflowService.deleteCashFlow(recUid);
		return respBean;

	}

	@RequestMapping(value = "/mod", method = RequestMethod.POST, consumes = { "application/json" }, produces = { "application/json;charset=UTF-8" })
	public @ResponseBody BaseRespBean CashFlowUpdate(HttpSession httpSession,
			@Valid @RequestBody CashFlowUpdateReqBean reqBean,
			BindingResult bindingResult) throws IOException {
		BaseRespBean respBean = new BaseRespBean();
		if (bindingResult.hasErrors()) {
			log.warn("bindingResult has error");
		...
		Map<String, Object> paramMap = BeanUtil.beanToMap(reqBean);
		respBean = cashflowService.updateCashFlow(paramMap);
		return respBean;

	}

	@RequestMapping(value = "/detail", method = RequestMethod.POST, consumes = { "application/json" }, produces = { "application/json;charset=UTF-8" })
	public @ResponseBody BaseRespBean CashFlowDetail(HttpSession httpSession,
			@Valid @RequestBody RecUidReqBean reqBean,
			BindingResult bindingResult) throws IOException {
		BaseRespBean respBean = new BaseRespBean();
		if (bindingResult.hasErrors()) {
		...
		String recUid = reqBean.getRecUid();
		respBean = cashflowService.getCashFlowById(recUid);
		return respBean;
	}
