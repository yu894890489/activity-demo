...
import com.ut.scf.reqbean.office.documentDownloadAddReqBean;
import com.ut.scf.reqbean.office.documentDownloadListReqBean;
import com.ut.scf.reqbean.office.documentDownloadUpdateReqBean;
...
public class documentDownloadController extends BaseJsonController{
	private static final Logger log = LoggerFactory
			.getLogger(documentDownloadController.class);
	@Resource
	private documentDownloadService documentDownloadService;
	@RequestMapping(value = "/list", method = RequestMethod.POST, consumes = { "application/json" }, produces = { "application/json;charset=UTF-8" })
	public @ResponseBody BaseRespBean documentDownloadList(
			HttpSession httpSession,
			@Valid @RequestBody documentDownloadListReqBean reqBean,
	...
	/*	if (roleTypeSession == ScfBizConsts.ROLE_TYPE_FACTOR) {
		...
		respBean = documentDownloadService.getdocumentDownloadList(paramMap, page);
		return respBean;
	}
	
	@RequestMapping(value = "/add", method = RequestMethod.POST, consumes = { "application/json" }, produces = { "application/json;charset=UTF-8" })
	public @ResponseBody BaseRespBean documentDownloadAdd(
			HttpSession httpSession,
			@Valid @RequestBody documentDownloadAddReqBean reqBean,
			BindingResult bindingResult) throws IOException {
		BaseRespBean respBean = new BaseRespBean();
		if (bindingResult.hasErrors()) {
			log.warn("bindingResult has error");
	...
	public @ResponseBody BaseRespBean documentDownloadDelete(
		...
		respBean = documentDownloadService.deletedocumentDownload(recUid);
		return respBean;
	}
	
	@RequestMapping(value = "/mod", method = RequestMethod.POST, consumes = { "application/json" }, produces = { "application/json;charset=UTF-8" })
	public @ResponseBody BaseRespBean documentDownloadUpdate(
			HttpSession httpSession,
			@Valid @RequestBody documentDownloadUpdateReqBean reqBean,
			BindingResult bindingResult) throws IOException {
		BaseRespBean respBean = new BaseRespBean();
		if (bindingResult.hasErrors()) {
			log.warn("bindingResult has error");
		...
		Map<String, Object> paramMap = BeanUtil.beanToMap(reqBean);
		respBean = documentDownloadService.updatedocumentDownload(paramMap);
		log.debug("documentDownload respBean : {}", respBean);
		return respBean;
	}
	
	@RequestMapping(value = "/detail", method = RequestMethod.POST, consumes = { "application/json" }, produces = { "application/json;charset=UTF-8" })
	public @ResponseBody BaseRespBean documentDownloadDetail(
			HttpSession httpSession,
			@Valid @RequestBody RecUidReqBean reqBean,
			BindingResult bindingResult) throws IOException {
		BaseRespBean respBean = new BaseRespBean();
		...
		String recUid = reqBean.getRecUid();
		respBean = documentDownloadService.getdocumentDownload(recUid);
		return respBean;
	}
}