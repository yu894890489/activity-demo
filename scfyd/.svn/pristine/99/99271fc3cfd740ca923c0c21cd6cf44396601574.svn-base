package com.ut.scf.reqbean.crm;

import com.ut.scf.reqbean.BaseReqBean;

public class ShareHolderInfoAddReqBean extends BaseReqBean{
    private String shareName;
    private String shareType;
    private Double subscribedCapital;
    private Double paidInCapital;
    private Double shareProportion;
    private Double registeredCapital;
    private Double registeredCapitalProportion;
    private String corpId;
    private String note;
    
	public String getShareName() {
		return shareName;
	}

	public void setShareName(String shareName) {
		this.shareName = shareName;
	}

	public String getShareType() {
		return shareType;
	}

	public void setShareType(String shareType) {
		this.shareType = shareType;
	}

	public Double getSubscribedCapital() {
		return subscribedCapital;
	}

	public void setSubscribedCapital(Double subscribedCapital) {
		this.subscribedCapital = subscribedCapital;
	}

	public Double getPaidInCapital() {
		return paidInCapital;
	}

	public void setPaidInCapital(Double paidInCapital) {
		this.paidInCapital = paidInCapital;
	}

	public Double getShareProportion() {
		return shareProportion;
	}

	public void setShareProportion(Double shareProportion) {
		this.shareProportion = shareProportion;
	}

	public String getCorpId() {
		return corpId;
	}

	public void setCorpId(String corpId) {
		this.corpId = corpId;
	}

	public String getNote() {
		return note;
	}

	public void setNote(String note) {
		this.note = note;
	}

	public Double getRegisteredCapital() {
		return registeredCapital;
	}

	public void setRegisteredCapital(Double registeredCapital) {
		this.registeredCapital = registeredCapital;
	}

	public Double getRegisteredCapitalProportion() {
		return registeredCapitalProportion;
	}

	public void setRegisteredCapitalProportion(Double registeredCapitalProportion) {
		this.registeredCapitalProportion = registeredCapitalProportion;
	}
	
}