package com.ut.scf.respbean.bpm;

import com.ut.scf.respbean.BaseRespBean;
/**
 * 合同申请
 * @author zhangyx
 *
 */
public class ContractAddRespBean extends BaseRespBean {
	
	/**
	 * 项目名称
	 */
	private String proName;
	
	/**
	 * 是否有追
	 */
	private String chaseFlg;
	
	/**
	 * 保理类型
	 */
	private String factorType;
	
	/**
	 * 立项人员名称
	 */
	private String proMembName;
	
	/**
	 * 立项发起日期
	 */
	private String proMakeDate;
	
	/**
	 * 关联买方名称
	 */
	private String relBuyName;
	
	/**
	 * 关联卖方名称
	 */
	private String relSaleName;
	
	/**
	 * 关联企业名称
	 */
	private String relSaleCorpName;
	
	/**
	 * 备注
	 */
	private String note;
	
	/**
	 * 合同编号
	 */
	private String contractNo;
	
	/**
	 * 公司名称
	 */
	private String corpNm;
	
	/**
	 * 卖方公司id
	 */
	private String selCorpId;
	
	/**
	 * 营业执照号码
	 */
	private String busiLicenceNo;
	
	/**
	 * 买卖关系
	 */
	private String busiRela;
	
	/**
	 * 发票编号
	 */
	private String invNo;
	/**
	 * 发票金额
	 */
	private String invAmt;
	
	/**
	 * 应收账款
	 */
	private String arAmt;
	
	/**
	 * 应收账款说明
	 */
	private String arNote;
	
	/**
	 * 应收账款受让款
	 */
	private String arTransfAmt;
	
	/**
	 * 应收账款受让款说明
	 */
	private String arTransfNote;
	
	/**
	 * 保证金
	 */
	private String earnestMoney;
	
	/**
	 * 服务费
	 */
	private String serviceAmt;
	
	/**
	 * 应收账款转让通知
	 */
	private String arTransfNotice;
	
	/**
	 * 担保人名称
	 */
	private String guarantorName;
	
	/**
	 * 担保品名称
	 */
	private String guaranteeName;
	
	/**
	 * 担保方式  
	 */
	private String guaranteeMethod;
	
	/**
	 * 币种  
	 */
	private String ccy;
	
	/**
	 * 担保价值
	 */
	private String guaranteeValue;
	
	/**
	 * 担保附件上传  
	 */
	private String guaranteeFile;
	/**
	 * 收益补偿
	 */
	private String inComeMakeUp;
	
	/**
	 * 合同生效条款
	 */
	private String cntEffectiveTerms;
	/**
	 * 特别条款说明
	 */
	private String specTermsNotice;
	/**
	 * 合同版本
	 */
	private String cntVersion;
	/**
	 * 合同生效日期
	 */
	private String cntEffectDate;
	/**
	 * 合同状态
	 */
	private String cntState;
	/**
	 * 合同结束日期
	 */
	private String cntEndDate;
	
	/**
	 * 合同有效性
	 */
	private String cntEffectiveness;
	
	/**
	 * 合同附件上传
	 */
	private String cntFiles;
	
	/**
	 * 融资期限(月)
	 */
	private String loanLimit;
	
	/**
	 * 融资比例
	 */
	private String loanPct;
	
	/**
	 * 融资金额(元)
	 */
	private String loanAmt;
	
	/**
	 * 基准利率
	 */
	private String standardRate;
	
	/**
	 * 利率上浮比例
	 */
	private String rateFloatPct;
	
	/**
	 * 手续费
	 */
	private String commission;
	
	/**
	 * 其他费用
	 */
	private String otherCost;
	
	/**
	 * 罚息率
	 * Penalty for interest rate
	 */
	private String penaltyIntRate;
	
	/**
	 * 管理费
	 */
	private String managementFee;
	
	/**
	 * 管理费率
	 */
	private String managementFeeRate;
	
	/**
	 * 违约金
	 */
	private String penalty;
	
	/**
	 * 融资期数
	 */
	private String loanPeriod;
	
	/**
	 * 还款企业
	 */
	private String repaymentCorp;
	
	/**
	 * 还款时间
	 */
	private String repaymentDate;
	
	/**
	 * 还款人
	 */
	private String repaymentPerson;
	
	/**
	 * 还款企业账户
	 */
	private String repaymentCorpAcount;
	
	/**
	 * 还款计划
	 */
	private String repaymentPlan;
	
	/**
	 * 还款金额
	 */
	private String repaymentAmt;
	
	/**
	 * 还款类型
	 */
	private String repaymentType;
	
	/**
	 * 还款时长
	 */
	private String repaymentTimes;
	
	public String getProName() {
		return proName;
	}

	public void setProName(String proName) {
		this.proName = proName;
	}

	public String getChaseFlg() {
		return chaseFlg;
	}

	public void setChaseFlg(String chaseFlg) {
		this.chaseFlg = chaseFlg;
	}

	public String getFactorType() {
		return factorType;
	}

	public void setFactorType(String factorType) {
		this.factorType = factorType;
	}

	public String getProMembName() {
		return proMembName;
	}

	public void setProMembName(String proMembName) {
		this.proMembName = proMembName;
	}

	public String getProMakeDate() {
		return proMakeDate;
	}

	public void setProMakeDate(String proMakeDate) {
		this.proMakeDate = proMakeDate;
	}

	public String getRelBuyName() {
		return relBuyName;
	}

	public void setRelBuyName(String relBuyName) {
		this.relBuyName = relBuyName;
	}

	public String getRelSaleName() {
		return relSaleName;
	}

	public void setRelSaleName(String relSaleName) {
		this.relSaleName = relSaleName;
	}

	public String getRelSaleCorpName() {
		return relSaleCorpName;
	}

	public void setRelSaleCorpName(String relSaleCorpName) {
		this.relSaleCorpName = relSaleCorpName;
	}

	public String getNote() {
		return note;
	}

	public void setNote(String note) {
		this.note = note;
	}

	public String getContractNo() {
		return contractNo;
	}

	public void setContractNo(String contractNo) {
		this.contractNo = contractNo;
	}

	public String getBusiRela() {
		return busiRela;
	}

	public void setBusiRela(String busiRela) {
		this.busiRela = busiRela;
	}

	public String getArAmt() {
		return arAmt;
	}

	public void setArAmt(String arAmt) {
		this.arAmt = arAmt;
	}

	public String getArNote() {
		return arNote;
	}

	public void setArNote(String arNote) {
		this.arNote = arNote;
	}

	public String getArTransfAmt() {
		return arTransfAmt;
	}

	public void setArTransfAmt(String arTransfAmt) {
		this.arTransfAmt = arTransfAmt;
	}

	public String getArTransfNote() {
		return arTransfNote;
	}

	public void setArTransfNote(String arTransfNote) {
		this.arTransfNote = arTransfNote;
	}

	public String getEarnestMoney() {
		return earnestMoney;
	}

	public void setEarnestMoney(String earnestMoney) {
		this.earnestMoney = earnestMoney;
	}

	public String getServiceAmt() {
		return serviceAmt;
	}

	public void setServiceAmt(String serviceAmt) {
		this.serviceAmt = serviceAmt;
	}

	public String getArTransfNotice() {
		return arTransfNotice;
	}

	public void setArTransfNotice(String arTransfNotice) {
		this.arTransfNotice = arTransfNotice;
	}

	public String getGuarantorName() {
		return guarantorName;
	}

	public void setGuarantorName(String guarantorName) {
		this.guarantorName = guarantorName;
	}

	public String getGuaranteeName() {
		return guaranteeName;
	}

	public void setGuaranteeName(String guaranteeName) {
		this.guaranteeName = guaranteeName;
	}

	public String getGuaranteeMethod() {
		return guaranteeMethod;
	}

	public void setGuaranteeMethod(String guaranteeMethod) {
		this.guaranteeMethod = guaranteeMethod;
	}

	public String getCcy() {
		return ccy;
	}

	public void setCcy(String ccy) {
		this.ccy = ccy;
	}

	public String getGuaranteeValue() {
		return guaranteeValue;
	}

	public void setGuaranteeValue(String guaranteeValue) {
		this.guaranteeValue = guaranteeValue;
	}

	public String getGuaranteeFile() {
		return guaranteeFile;
	}

	public void setGuaranteeFile(String guaranteeFile) {
		this.guaranteeFile = guaranteeFile;
	}

	public String getInComeMakeUp() {
		return inComeMakeUp;
	}

	public void setInComeMakeUp(String inComeMakeUp) {
		this.inComeMakeUp = inComeMakeUp;
	}

	public String getCntEffectiveTerms() {
		return cntEffectiveTerms;
	}

	public void setCntEffectiveTerms(String cntEffectiveTerms) {
		this.cntEffectiveTerms = cntEffectiveTerms;
	}

	public String getSpecTermsNotice() {
		return specTermsNotice;
	}

	public void setSpecTermsNotice(String specTermsNotice) {
		this.specTermsNotice = specTermsNotice;
	}

	public String getCntVersion() {
		return cntVersion;
	}

	public void setCntVersion(String cntVersion) {
		this.cntVersion = cntVersion;
	}

	public String getCntEffectDate() {
		return cntEffectDate;
	}

	public void setCntEffectDate(String cntEffectDate) {
		this.cntEffectDate = cntEffectDate;
	}

	public String getCntState() {
		return cntState;
	}

	public void setCntState(String cntState) {
		this.cntState = cntState;
	}

	public String getCntEndDate() {
		return cntEndDate;
	}

	public void setCntEndDate(String cntEndDate) {
		this.cntEndDate = cntEndDate;
	}

	public String getCntEffectiveness() {
		return cntEffectiveness;
	}

	public void setCntEffectiveness(String cntEffectiveness) {
		this.cntEffectiveness = cntEffectiveness;
	}

	public String getCntFiles() {
		return cntFiles;
	}

	public void setCntFiles(String cntFiles) {
		this.cntFiles = cntFiles;
	}

	public String getLoanLimit() {
		return loanLimit;
	}

	public void setLoanLimit(String loanLimit) {
		this.loanLimit = loanLimit;
	}

	public String getLoanPct() {
		return loanPct;
	}

	public void setLoanPct(String loanPct) {
		this.loanPct = loanPct;
	}

	public String getLoanAmt() {
		return loanAmt;
	}

	public void setLoanAmt(String loanAmt) {
		this.loanAmt = loanAmt;
	}

	public String getStandardRate() {
		return standardRate;
	}

	public void setStandardRate(String standardRate) {
		this.standardRate = standardRate;
	}

	public String getRateFloatPct() {
		return rateFloatPct;
	}

	public void setRateFloatPct(String rateFloatPct) {
		this.rateFloatPct = rateFloatPct;
	}

	public String getCommission() {
		return commission;
	}

	public void setCommission(String commission) {
		this.commission = commission;
	}

	public String getOtherCost() {
		return otherCost;
	}

	public void setOtherCost(String otherCost) {
		this.otherCost = otherCost;
	}

	public String getPenaltyIntRate() {
		return penaltyIntRate;
	}

	public void setPenaltyIntRate(String penaltyIntRate) {
		this.penaltyIntRate = penaltyIntRate;
	}

	public String getManagementFee() {
		return managementFee;
	}

	public void setManagementFee(String managementFee) {
		this.managementFee = managementFee;
	}

	public String getManagementFeeRate() {
		return managementFeeRate;
	}

	public void setManagementFeeRate(String managementFeeRate) {
		this.managementFeeRate = managementFeeRate;
	}

	public String getPenalty() {
		return penalty;
	}

	public void setPenalty(String penalty) {
		this.penalty = penalty;
	}

	public String getLoanPeriod() {
		return loanPeriod;
	}

	public void setLoanPeriod(String loanPeriod) {
		this.loanPeriod = loanPeriod;
	}

	public String getRepaymentCorp() {
		return repaymentCorp;
	}

	public void setRepaymentCorp(String repaymentCorp) {
		this.repaymentCorp = repaymentCorp;
	}

	public String getRepaymentDate() {
		return repaymentDate;
	}

	public void setRepaymentDate(String repaymentDate) {
		this.repaymentDate = repaymentDate;
	}

	public String getRepaymentPerson() {
		return repaymentPerson;
	}

	public void setRepaymentPerson(String repaymentPerson) {
		this.repaymentPerson = repaymentPerson;
	}

	public String getRepaymentCorpAcount() {
		return repaymentCorpAcount;
	}

	public void setRepaymentCorpAcount(String repaymentCorpAcount) {
		this.repaymentCorpAcount = repaymentCorpAcount;
	}

	public String getRepaymentPlan() {
		return repaymentPlan;
	}

	public void setRepaymentPlan(String repaymentPlan) {
		this.repaymentPlan = repaymentPlan;
	}

	public String getRepaymentAmt() {
		return repaymentAmt;
	}

	public void setRepaymentAmt(String repaymentAmt) {
		this.repaymentAmt = repaymentAmt;
	}

	public String getRepaymentType() {
		return repaymentType;
	}

	public void setRepaymentType(String repaymentType) {
		this.repaymentType = repaymentType;
	}

	public String getRepaymentTimes() {
		return repaymentTimes;
	}

	public void setRepaymentTimes(String repaymentTimes) {
		this.repaymentTimes = repaymentTimes;
	}

	public String getInvNo() {
		return invNo;
	}

	public void setInvNo(String invNo) {
		this.invNo = invNo;
	}

	public String getInvAmt() {
		return invAmt;
	}

	public void setInvAmt(String invAmt) {
		this.invAmt = invAmt;
	}

	public String getCorpNm() {
		return corpNm;
	}

	public void setCorpNm(String corpNm) {
		this.corpNm = corpNm;
	}

	public String getBusiLicenceNo() {
		return busiLicenceNo;
	}

	public void setBusiLicenceNo(String busiLicenceNo) {
		this.busiLicenceNo = busiLicenceNo;
	}

	public String getSelCorpId() {
		return selCorpId;
	}

	public void setSelCorpId(String selCorpId) {
		this.selCorpId = selCorpId;
	}
	
}
