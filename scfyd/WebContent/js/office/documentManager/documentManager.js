...
function initTable() { 
	$('#documentTable').bootstrapTable('destroy');  
...
function detailFun(row,isEdit) {
	window.location.href="documentDetail.html?id="+row.recUid;
}
...
function modFun(row,isEdit) {
 ...
 //   $("#type").find("option[value='"+row.type+"']").attr("selected", true);
    $('#editor').html(row.documentContent);
...
function saveUser() {
	var documentContent = $('#editor').html(); 
