...
public class LoginController extends BaseJsonController {
	...
	public @ResponseBody BaseRespBean login(HttpServletRequest request,
		...
		if (respBean.getResult() == ErrorCodeEnum.SUCCESS.getValue()) {
			...
			httpSession
					.removeAttribute(ScfConsts.SESSION_LOGIN_CODE_EXPIRE_TIME);
		}
	...
	public @ResponseBody BaseRespBean loginPhone(HttpServletRequest request,
			HttpSession httpSession,
		...
		if (respBean.getResult() == ErrorCodeEnum.SUCCESS.getValue()) {
			...
			httpSession
					.removeAttribute(ScfConsts.SESSION_LOGIN_PHONE_CODE_EXPIRE_TIME);
		}
	...
	public @ResponseBody BaseRespBean loginPhoneCode(
		...
		// 查找手机号对应的用户是否存在
		int countUserByPhone = userOperService.countUserByPhone(reqBean
				.getPhone());
		...
		try {
			AlibabaAliqinFcSmsNumSendResponse response = smsClient.execute(
					smsReq, "");
			...
			if (!(boolean) ((Map<?, ?>) ((Map<?, ?>) mapResp
					.get("alibaba_aliqin_fc_sms_num_send_response"))
					.get("result")).get("success")) {
		...
		httpSession.setAttribute(
				ScfConsts.SESSION_LOGIN_PHONE_CODE_EXPIRE_TIME,
				loginCodeExpireTime);
	...
	public @ResponseBody BaseRespBean signPhoneCode(HttpServletRequest request,
			HttpSession httpSession, @Valid @RequestBody PhoneReqBean reqBean,
			BindingResult bindingResult) throws IOException {
		BaseRespBean respBean = new BaseRespBean();
		if (bindingResult.hasErrors()) {
			log.warn("bindingResult has error");
		...
		smsReq.setSmsParam("{\"name\":'" + userNameSession + "',\"code\":'"
				+ phoneCode + "'}");
		try {
			AlibabaAliqinFcSmsNumSendResponse response = smsClient.execute(
					smsReq, "");
	...
	public @ResponseBody BaseRespBean signPhoneCheck(
			HttpServletRequest request, HttpSession httpSession,
			@Valid @RequestBody LoginPhoneReqBean reqBean,
			BindingResult bindingResult) throws IOException {
		BaseRespBean respBean = new BaseRespBean();
		...
		if (respBean.getResult() == ErrorCodeEnum.SUCCESS.getValue()) {
			...
			httpSession
					.removeAttribute(ScfConsts.SESSION_SIGN_PHONE_CODE_EXPIRE_TIME);
		}
	...
	public @ResponseBody BaseRespBean logout(HttpSession httpSession,
		...
		BaseRespBean respBean = userOperService.updateUserRoleStatus(userId,
				userRoleReqBean.getRoleId());
		if (httpSession != null) {
