...
import com.ut.scf.reqbean.finance.ReceiveAccountAddReqBean;
import com.ut.scf.reqbean.finance.ReceiveAccountListReqBean;
import com.ut.scf.reqbean.finance.ReceiveAccountUpdateReqBean;
...
public class ReceiveAccountController extends BaseJsonController{
	private static final Logger log = LoggerFactory
			.getLogger(ReceiveAccountController.class);
	@Resource
	private IReceiveAccountService receiveAccountService;
	@RequestMapping(value = "/list", method = RequestMethod.POST, consumes = { "application/json" }, produces = { "application/json;charset=UTF-8" })
	public @ResponseBody BaseRespBean receiveAccountList(
			HttpSession httpSession,
			@Valid @RequestBody ReceiveAccountListReqBean reqBean,
		...
		respBean = receiveAccountService.getReceiveAccountList(paramMap, page);
		return respBean;
	}
	@RequestMapping(value = "/add", method = RequestMethod.POST, consumes = { "application/json" }, produces = { "application/json;charset=UTF-8" })
	public @ResponseBody BaseRespBean receiveAccountAdd(
			HttpSession httpSession,
			@Valid @RequestBody ReceiveAccountAddReqBean reqBean,
			BindingResult bindingResult) throws IOException {
		BaseRespBean respBean = new BaseRespBean();
		if (bindingResult.hasErrors()) {
			log.warn("bindingResult has error");
	...
	public @ResponseBody BaseRespBean receiveAccountDelete(
		...
		respBean = receiveAccountService.deleteReceiveAccount(recUid);
		return respBean;
	}
	@RequestMapping(value = "/mod", method = RequestMethod.POST, consumes = { "application/json" }, produces = { "application/json;charset=UTF-8" })
	public @ResponseBody BaseRespBean receiveAccountUpdate(
			HttpSession httpSession,
			@Valid @RequestBody ReceiveAccountUpdateReqBean reqBean,
			BindingResult bindingResult) throws IOException {
		BaseRespBean respBean = new BaseRespBean();
		if (bindingResult.hasErrors()) {
			log.warn("bindingResult has error");
		...
		Map<String, Object> paramMap = BeanUtil.beanToMap(reqBean);
		respBean = receiveAccountService.updateReceiveAccount(paramMap);
		return respBean;
	}
}
